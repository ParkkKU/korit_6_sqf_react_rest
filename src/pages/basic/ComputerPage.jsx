...
function ComputerPage(props) {
    ...
    const [ sParams, setParams ] = useState({
        company: "",
        cpu: "",
    });
    ...
    const requestComputerList = async () => {
        try {
            const response = await axios.get("http://localhost:8080/api/v1/computers", {params: sParams});
    ...
    return (
        <div>
            ...
            <div css={layout}>
                <h2>목록</h2>
                <p>    
                    <input type='text' name='company' onChange={handleSearchInputChange} value={sParams.company} placeholder='제조사' />
                    <input type='text' name='cpu' onChange={handleSearchInputChange} value={sParams.cpu} placeholder='CPU' />
